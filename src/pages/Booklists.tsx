...
export default function Booklists() {
  ...
  const [activeTab, setActiveTab] = useState<'routine' | 'booklist' | 'combined'>('booklist');
  ...
  const fetchData = async () => {
    try {
      ...
      const snapshot = await getDocs(qBooklists);
      const dataBooklists = snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
  ...
  const displayExamName = useMemo(() => {
    const withExamName = filteredRoutines.length > 0
      ? filteredRoutines.filter((r) => r.examName)
  ...
  const handleRoutineDownloadPNG = () => generateRoutineImageAndDownload('jpg');
  const handleRoutinePrint = () => generateRoutineImageAndDownload('pdf');

  const generateRoutineImageAndDownload = async (type: 'pdf' | 'jpg') => {
    ...
    if (printContent) {
      ...
      .md\\:text-5xl { font-size: 3.5rem !important; line-height: 1 !important; }
    ...
    `;
      ...
      try {
        ...
        console.error("Error generating routine screenshot/PDF", err);
      } finally {
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 sm:px-6 rounded-2xl border border-slate-100 shadow-sm print:hidden gap-4">
                        ...
                        <div className="flex flex-wrap w-full sm:w-auto gap-2">
                          ...
                          <button
                            onClick={handleRoutineDownloadPNG}
                            ...
                            <div className="table-wrapper overflow-x-auto sm:overflow-visible rounded-xl border border-slate-200 shadow-sm">
                              <table className="w-full text-left border-collapse min-w-[500px] sm:min-w-0 bg-white">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white text-slate-700">
                                  {filteredRoutines.map((subject, idx) => (
                                    <tr
                                      key={subject.id}
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 sm:px-6 rounded-2xl border border-slate-100 shadow-sm print:hidden gap-4">
                        ...
                        <div className="flex flex-wrap w-full sm:w-auto gap-2">
                          <button
                            onClick={handleShare}
                            className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 px-4 py-2 rounded-xl text-sm font-semibold transition-colors cursor-pointer"
                          >
