...
export default function Booklists() {
  ...
  useEffect(() => {
    if (activeCurriculum && booklists.length > 0) {
      const regsForCurr = Array.from(new Set(booklists.filter(r => (r.curriculum || 'Unknown') === activeCurriculum).map(r => r.regulation || 'Unknown'))).sort().reverse();
      if (!regsForCurr.includes(activeRegulation || '')) {
        setActiveRegulation(regsForCurr.length > 0 ? regsForCurr[0] : null);
      }
    }
  }, [activeCurriculum, booklists]);

  const filteredBooklists = booklists.filter(
    r => (r.curriculum || 'Unknown') === activeCurriculum && (r.regulation || 'Unknown') === activeRegulation
  );
  
  const curriculums = Array.from(new Set(booklists.map(r => r.curriculum || 'Unknown'))).sort().reverse();
  const regulations = Array.from(new Set(booklists.filter(r => (r.curriculum || 'Unknown') === activeCurriculum).map(r => r.regulation || 'Unknown'))).sort().reverse();
    ...
    <div className="flex flex-col items-center w-full px-4 sm:px-6 py-12 font-sans">
      <div className="w-full max-w-4xl space-y-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center print:hidden">
          ...
          <p className="text-gray-500 mb-6">Browse and download booklists by department and semester.</p>
